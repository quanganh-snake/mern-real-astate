				...
				<>
					<Swiper navigation>
						{listing?.imageUrls.map((url) => (
							<SwiperSlide key={url}>
								<div className="w-full h-[550px]" style={{ background: `url(${url}) center no-repeat`, backgroundSize: "cover" }}></div>
							</SwiperSlide>
						))}
					</Swiper>
					...
					<div className="flex flex-col max-w-4xl mx-auto p-3 my-7 gap-4">
						...
						{currentUser && listing?.userRef !== currentUser._id && !contact && (
							<button type="button" onClick={() => setContact(true)} className="bg-slate-700 text-white rounded-lg uppercase hover:opacity-95 p-3">
								Contact landlord
							</button>
						)}
